...
подключиться специальной программой к одному из источников передачи или приёма данных. Такие программы называются [снифферами][sniffer].
...
Вот тут вспоминаем о такой штуке как ssh порт-форвардинг (или ssh-туннелинг).
 Нам потребуется REMOTE (который с ключом `-R`) и соответственно ssh-доступ к машинке. 
 Это позволит тестовому сервису обращаться на свой локальный порт, а нам слушать свой. И все будет работать. 
